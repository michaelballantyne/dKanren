...
(define (filter-branches disjunction constraint-store)
  (define new-children
    (filter-map
      (match-lambda
        [(conj #f children #f)
         (define-values (new-constraint-store new-disjunctions)
           (reduce-conj constraint-store children #f))
         (and new-constraint-store
              (conj #f children #f))])
      (disj-children disjunction)))
  (and (pair? new-children) (disj new-children)))

(define (filter-disjunctions remaining constraint-store acc)
...
(define (step-state current-state)
  ...
  (if (pair? unreduced)
    (match-let ([(conj constraint-store unsorted disjunctions)
                 (first unreduced)])
      (define-values (new-constraint-store new-disjunctions)
        (reduce-conj constraint-store unsorted (disjq:queue)))
      (define filtered-disjunctions
        (and new-disjunctions
             (filter-disjunctions new-disjunctions new-constraint-store disjunctions)))
      (cond
